...
public class Organism extends Matter {
	 ...
	 public List<Integer> getFoodInRange(int scanRange) {
		  ...
		  // conditional.
		  Coordinate cornerTop = new Coordinate(widthSub - scanRange,
				    heightSub - scanRange);
		  ...
		  for (int i = cornerTop.getX(); i <= cornerBottom.getX(); i++) {
			   for (int j = cornerTop.getY(); j <= cornerBottom.getY(); j++) {
				    try {
						...
						// map
						Pair<Integer, Character> space = locationMap.get(
								  i, j);
						if (space.getSnd() == 'h'
								  || space.getSnd() == 'p')
	 ...
	 public void newLocation() {
		  int x = location.getX();
		  int y = location.getY();
		  LocationMap.getInstance().newLocation(location, width, height, id,
				    'o');
	 }
	 ...
	 public void moveNorth(LinkedList<Organism> organisms, boolean wasPushed) {
		  ...
		  /*map.setRange(width, height, 'w');
		  map.setWrapAround(width, height);*/

		  // if the next move is available.
		  try {
			   /*if(map.canSpawn(location.getX(), location.getY() - 1)){
			   	// move there.
	 ...
	 /**
	  * Takes an organism index and a list of Organisms and decreases the
	  * health of the organism that is attacked.
	  * 
	  * @param orgIndex
	  * @param anOrgList
	  */
	 ...
	 public void paint(Graphics g) {
		  ...
		  g.fillRect((int) this.location.getX() - (width / 2),
				    (int) this.location.getY() - (height / 2), width,
				    height);
	 }
	 	...
	 	if (facingRight) {
	 		if (getHealth() <= 0) {
	 			g.drawImage(ninja_dead, location.getX() - 2 * Organism.width
	 					/ 2, location.getY() - 2 * Organism.height / 2,
	 					2 * Organism.width, 2 * Organism.height, null);
	 		} else {
	 			if (currentAction == 'e') {
	 				g.drawImage(ninja_eat, location.getX() - 2 * Organism.width
	 						/ 2, location.getY() - 2 * Organism.height / 2,
	 						2 * Organism.width, 2 * Organism.height, null);
	 			} else if (currentAction == 'a') {
	 				g.drawImage(ninja_attack, location.getX() - 2
	 						* Organism.width / 2, location.getY() - 2
	 						* Organism.height / 2, 2 * Organism.width,
	 						2 * Organism.height, null);
	 			} else if (currentAction == 'p') {
	 				g.drawImage(ninja_push, location.getX() - 2
	 						* Organism.width / 2, location.getY() - 2
	 						* Organism.height / 2, 2 * Organism.width,
	 						2 * Organism.height, null);
	 			} else {
	 				if (swapImage) {
	 					g.drawImage(ninja_walk1, location.getX() - 2
	 							* Organism.width / 2, location.getY() - 2
	 							* Organism.height / 2, 2 * Organism.width,
	 							2 * Organism.height, null);
	 				} else {
	 					g.drawImage(ninja_walk2, location.getX() - 2
	 							* Organism.width / 2, location.getY() - 2
	 							* Organism.height / 2, 2 * Organism.width,
	 							2 * Organism.height, null);
	 				}
	 		...
	 		if (getHealth() <= 0) {
	 			g.drawImage(ninja_dead_inv, location.getX() - 2
	 					* Organism.width / 2, location.getY() - 2
	 					* Organism.height / 2, 2 * Organism.width,
	 					2 * Organism.height, null);
	 		} else {
	 			if (currentAction == 'e') {
	 				g.drawImage(ninja_eat_inv, location.getX() - 2
	 						* Organism.width / 2, location.getY() - 2
	 						* Organism.height / 2, 2 * Organism.width,
	 						2 * Organism.height, null);
	 			} else if (currentAction == 'a') {
	 				g.drawImage(ninja_attack_inv, location.getX() - 2
	 						* Organism.width / 2, location.getY() - 2
	 						* Organism.height / 2, 2 * Organism.width,
	 						2 * Organism.height, null);
	 			} else if (currentAction == 'p') {
	 				g.drawImage(ninja_push_inv, location.getX() - 2
	 						* Organism.width / 2, location.getY() - 2
	 						* Organism.height / 2, 2 * Organism.width,
	 						2 * Organism.height, null);
	 			} else {
	 				if (swapImage) {
	 					g.drawImage(ninja_walk1_inv, location.getX() - 2
	 							* Organism.width / 2, location.getY() - 2
	 							* Organism.height / 2, 2 * Organism.width,
	 							2 * Organism.height, null);
	 				} else {
	 					g.drawImage(ninja_walk2_inv, location.getX() - 2
	 							* Organism.width / 2, location.getY() - 2
	 							* Organism.height / 2, 2 * Organism.width,
	 							2 * Organism.height, null);
	 				}
