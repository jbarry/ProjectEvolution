...
public class GEP {
	...
	public LinkedList<Organism> newGeneration(LinkedList<Organism> anOrgList) {
		...
		LinkedList<Chromosome> chromList = makeChromList(tournament(partnerSelect(anOrgList)));
		rotation(chromList);
		mutation(chromList);
	...
	public LinkedList<Organism> newGenerationTest(LinkedList<Organism> anOrgList) {
		...
		if (doElitism) {
			...
			eliteList = (LinkedList<Chromosome>) assembleElites((LinkedList<Organism>) anOrgList
					.clone());
			System.out.println("The elite'sIds are: ");
	...
	public LinkedList<Pair<Chromosome, Chromosome>> mateSelect(
		...
		while (!competitors.isEmpty()) {
			Chromosome chrom1 = competitors.remove(ran.nextInt(competitors
					.size()));
			Chromosome chrom2 = competitors.remove(ran.nextInt(competitors
					.size()));
	...
	/*public static void main(String[] args) {
		...
		GEP gep2 = new GEP(1.00, 1.00, 1.00, 1.00, 1.00, 2, false, false); // Elitism
																		   // ctor.
