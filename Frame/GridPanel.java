...
public class GridPanel extends JPanel {
      ...
      public GridPanel(final GUI aGui) {
	    ...
	    Runnable r = new Runnable() {
		  ...
		  public void run() {
			...
			t =
				    new javax.swing.Timer(lengthTimeStep,
						new ActionListener() {
						      public void actionPerformed(ActionEvent e) {
							    if (timePassed < lengthGeneration) {
								  ...
								  simulateStepAstarWithoutEatWithClosedListInOrgData(2);
								  repaint();
      ...
      public void attack2(Organism org, ArrayList<Organism> aOrgsUsed) {
	    ArrayList<Integer> surroundingOrganisms =
			org.getSurroundingObjects('o', 1);
	    surroundingOrganisms.remove((Integer) org.getId());
	    if (surroundingOrganisms.size() > 0) {
		  int index =
			      surroundingOrganisms
					  .get(ran.nextInt(surroundingOrganisms
						      .size()));
      ...
      public boolean eat(Organism org) {
	    LinkedList<Food> foodToEatList =
			(LinkedList<Food>) collectFoodInRange2(org, 2);
	    Food foodToEat;
	    if (!foodToEatList.isEmpty())
		  foodToEat =
			      foodToEatList.get(ran.nextInt(foodToEatList
					  .size()));
	    else
      ...
      public void push(Organism org) {
	    ArrayList<Integer> surroundingOrganismsPush =
			org.getSurroundingObjects('o', 1);
	    surroundingOrganismsPush.remove((Integer) org.getId());
	    if (surroundingOrganismsPush.size() > 0) {
		  int index =
			      surroundingOrganismsPush.get(ran
					  .nextInt(surroundingOrganismsPush
						      .size()));
			      ...
			      if (org.getHealth() > 0) {
				    ...
				    double orgY =
						norm.normalize(org
							    ...
							    .getY());
				    double health =
						norm.normalize(org.getHealth());
				    double numSurroundingOrgs =
						norm.normalize(org
							    .getSurroundingObjects('o',
									5)
							    .size() - 1);
				    Chromosome chrom = org.getChromosome();
				    Pair<Integer, Double> bestEval1 =
						new Pair<Integer, Double>(0,
							    0.0);
				    Pair<Integer, Double> bestEval2 =
						new Pair<Integer, Double>(1,
							    0.0);
      ...
      public void initializeAstar() {
	    ...
	    locationMap.clearLocations();
	    norm =
			new Normalizer(new Pair<Double, Double>(-600.0, 600.0),
	    ...
	    locationMap.placeFoods(food);
	    g =
			new GEP(organisms, 0.75, 0.01, 0.01, 0.75, 0.75, 1,
				    false, false);
      }
      ...
      private void simulateStepAstarWithoutEatWithClosedListInOrgData(int numActions) {
	    Collections.shuffle(shuffleIds);
	    // Loop through Organisms.
	    mainLoop: for (int i = 0; i < shuffleIds.size(); i++) { // mainLoop.
		  // Get an Organism corresponding the the ids in the shuffleIds
		  ...
		  // Closed list.
		  ArrayList<Coordinate> closedList =
			      (ArrayList<Coordinate>) orgData.getClosedList();
		  closedList.add(org.getLocation());
		  // lastFoodSourceIndex holds the index of the last food source
		  // that was visited.
		  Integer lastFoodSourceIndex = 0;
       ...
       */
      private void moveAstar(Organism org,
		  Pair<Integer, Double> bestEval,
		  Food aFoodDestination) {
	    Coordinate nextMove =
			LocationMap.search(org.getLocation(),
      ...
      private void moveAstarClosedList(Organism org,
	    ...
	    StarQueue<Coordinate> sq =
			LocationMap.searchWithList(org.getLocation(),
				    aFoodDestination.getLocation(),
				    org.getId());
	    /*org.printId();
	    org.printLocation();*/
	    ...
	    if (sq.isEmpty())
		  System.out.println("sq is empty");
	    Coordinate nextMove = null;
	    boolean canMove;
	    getNextMove: while (!sq.isEmpty()) {
		  canMove = true;
		  nextMove = sq.remove();
		  ...
		  for (int i = 0; i < aClosedList.size(); i++) {
			...
			/*System.out.println("On closed list: " + closedX + ", " + closedY);*/
      ...
      private double evaluateGene(Organism org, Gene currentGene) {
	    ...
	    double numSurroundingOrgs =
			norm.normalize(org.getSurroundingObjects('o', 5).size() - 1);
      ...
      private double evaluateGeneFoodInRange(Organism org,
		  Gene currentGene,
		  Food aFood) {
	    HashMap<String, Double> environment = new HashMap<String, Double>();
	    double orgX = norm.normalize(org.getLocation().getX());
      ...
      private double evaluateGeneFoodInRangeAstar(Organism org,
	    ...
	    double distanceToFood =
			norm.normalize(LocationMap.distance(orgX,
				    orgY,
				    foodX,
				    foodY));
	    double health = norm.normalize(org.getHealth());
      ...
      private List<Food> collectFoodInRange2(Organism org, int aRange) {
	    ...
	    ArrayList<Integer> foodInRangeIds =
			(ArrayList<Integer>) org.getFoodInRange(aRange);
      ...
      private List<Food> collectFoodInRange2Test(Organism org) {
	    ...
	    // List of Ids of the food in range.
	    ArrayList<Integer> foodInRangeIds =
			(ArrayList<Integer>) org.getFoodInRange(sightRange);
	    // List to be filled with actual food objects.
	    LinkedList<Food> foodInRange = new LinkedList<Food>();
	    // Fill the healthyFoodInRange list with food objects obtained from
	    // ids.
