...
public class OptionsPanel extends JPanel implements Runnable {
	...
	private JButton toggleGraphics;
	...
	public OptionsPanel(final GUI gui, final GridPanel simulation) {
		...
		ActionListener s = new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
					...
					try {
						...
						if (!orgWidthTxtBox.getText().equals("")) {
							...
							if (Integer.parseInt(orgWidthTxtBox.getText()) <= 20
								...
								Organism.width = Integer
										.parseInt(orgWidthTxtBox.getText());

								// if enter was pressed, but
								// height was not filled in.
								if (!orgWidthTxtBox.getText().equals("")
										&& orgHeightTxtBox.getText().equals("")) {
									// copy the width's input.
									Organism.height = Integer
											.parseInt(orgWidthTxtBox.getText());
								} else {
									// height was filled in
									if (Integer.parseInt(orgHeightTxtBox
											.getText()) <= 20
											&& Integer.parseInt(orgHeightTxtBox
													.getText()) > 0) {
										Organism.height = Integer
												.parseInt(orgHeightTxtBox
														.getText());
									} else {
										orgSizeLbl
												.setText("Number not 1 <= x <= 20");
										return;
									}
								}
							} else {
				...
				if (!mutationRateTxtBox.getText().equals("")) {
					try {
						if (Integer.parseInt(mutationRateTxtBox.getText()) >= 0
								&& Integer.parseInt(mutationRateTxtBox
										.getText()) <= 100) {
							GEP.mutProb = (double) Integer
									.parseInt(mutationRateTxtBox.getText()) / 100;
						} else {
				...
				if (!rotationRateTxtBox.getText().equals("")) {
					try {
						if (Integer.parseInt(rotationRateTxtBox.getText()) >= 0
								&& Integer.parseInt(rotationRateTxtBox
										.getText()) <= 100) {
							GEP.rotProb = (double) Integer
									.parseInt(rotationRateTxtBox.getText()) / 100;
						} else {
				...
				if (!selectionRateTxtBox.getText().equals("")) {
					try {
						if (Integer.parseInt(selectionRateTxtBox.getText()) >= 0
								&& Integer.parseInt(selectionRateTxtBox
										.getText()) <= 100) {
							GEP.tournProb = (double) Integer
									.parseInt(selectionRateTxtBox.getText()) / 100;
						} else {
							selectionRateLbl
									.setText("Number not 0 <= x <= 100");
							return;
						}
					} catch (NumberFormatException e) {
				...
				if (!crossoverRateTxtBox.getText().equals("")) {
					try {
						if (Integer.parseInt(crossoverRateTxtBox.getText()) >= 0
								&& Integer.parseInt(crossoverRateTxtBox
										.getText()) <= 100) {
							GEP.onePtProb = (double) Integer
									.parseInt(crossoverRateTxtBox.getText()) / 100;
							GEP.twoPtProb = (double) Integer
									.parseInt(crossoverRateTxtBox.getText()) / 100;
						} else {
							crossoverRateLbl
									.setText("Number not 0 <= x <= 100");
				...
				// Since this field starts the simulation, it should be
				// handled last.
				if (!numOrgsTxtBox.getText().equals("")) {
					try {
						if (Integer.parseInt(numOrgsTxtBox.getText()) <= 500
							...
							// if user confirmed, or no other
							// existing simulation.
							if (confirmed == JOptionPane.YES_OPTION) {
								// everything checks out, start
								// simulation.
								OptionsPanel.numOrganisms = Integer
										.parseInt(numOrgsTxtBox.getText());
								start.setText("New");
								// Make sure pause is set
								// correctly.
								MonitorPanel.simStatus.setText("Running");
								pause.setText("Pause");
								toggleEnabled(true);
								enableToggleGraphics();
		...
		toggleGraphics = new JButton();
		toggleGraphics.setLayout(null);	
		toggleGraphics.setEnabled(false);
		toggleGraphics.setText("Toggle Graphics");
		toggleGraphics.setSize(180, 20);
		toggleGraphics.setLocation(
				OptionsPanel.WIDTH / 2 - (toggleGraphics.getWidth() / 2),
				OptionsPanel.HEIGHT - (toggleGraphics.getHeight() + 5));
		add(toggleGraphics);

		ActionListener g = new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent arg0) {
				simulation.toggleGraphics();
			}
		};
		toggleGraphics.addActionListener(g);
		...
		/**
		 * MouseListener, handles ALL of the above textfields will restore
		 * default label when field is clicked.
		 */
				...
				if (key == KeyEvent.VK_ENTER) {
					...
					if (simulation.getTimer().isRunning()) {
						if (orgWidthTxtBox.isFocusOwner()
								|| orgHeightTxtBox.isFocusOwner()) {
							try {
								// if something was entered for
								// width at least.
								if (!orgWidthTxtBox.getText().equals("")) {
									// if input is not far too
									// large
									if (Integer.parseInt(orgWidthTxtBox
											.getText()) <= 20
											&& Integer.parseInt(orgWidthTxtBox
													.getText()) > 0) {
										// make organisms
										...
										// height was not
										// filled in.
										if (!orgWidthTxtBox.getText()
												.equals("")
												&& orgHeightTxtBox.getText()
														.equals("")) {
											// copy the
											// width's
											// input.
							...
							try {
								if (Integer.parseInt(mutationRateTxtBox
										.getText()) >= 0
										&& Integer.parseInt(mutationRateTxtBox
												.getText()) <= 100) {
									simulation
							...
							try {
								if (Integer.parseInt(rotationRateTxtBox
										.getText()) >= 0
										&& Integer.parseInt(rotationRateTxtBox
												.getText()) <= 100) {
									simulation
							...
							try {
								if (Integer.parseInt(selectionRateTxtBox
										.getText()) >= 0
										&& Integer.parseInt(selectionRateTxtBox
												.getText()) <= 100) {
									simulation
							...
							try {
								if (Integer.parseInt(crossoverRateTxtBox
										.getText()) >= 0
										&& Integer.parseInt(crossoverRateTxtBox
												.getText()) <= 100) {
									simulation
		...
		MouseListener sldrMouseListener = new MouseListener() {
			@Override
			public void mouseClicked(MouseEvent arg0) {
				timeStepSelectionSldr.setValue(myUI
						.valueForXPosition(timeStepSelectionSldr
								.getMousePosition().x));
			}
		...
		ChangeListener sldrChangeListener = new ChangeListener() {
			...
			public void stateChanged(ChangeEvent arg0) {
				...
				timeStepSelectionLbl.setText("Current Time Step: "
						+ timeStepSelectionSldr.getValue() + "ms");
			}
		};
	...
	public void enableToggleGraphics(){
		toggleGraphics.setEnabled(true);
	}
